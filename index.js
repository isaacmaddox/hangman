...
const RANDOM_URL = "https://random-word-api.herokuapp.com/word";
let word;
let wordArray;
let guessedLetters = [];
...
document.addEventListener("DOMContentLoaded", () => {
   ...
   setWordInput.addEventListener("keydown", (e) => {
      ...
      if (
         ...
         setWordInput.value.length === parseInt(setWordInput.getAttribute("maxlength")) &&
         !e.ctrlKey
      ) {
   ...
   setWordInput.addEventListener("keyup", (e) => {
      let dis =
         (setWordInput.value.length > 0 &&
            (setWordInput.value.length < 3 || setWordInput.value.length > MAX_WORD_LENGTH)) ||
         (randomWord === undefined && setWordInput.value.length === 0);
